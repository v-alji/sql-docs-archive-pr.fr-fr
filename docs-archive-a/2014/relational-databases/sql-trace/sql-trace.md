...
|**LoginSid** <sup>1</sup>|41|Numéro d'identification de sécurité (SID) de l'utilisateur connecté. Ces informations se trouvent dans la vue **sys.server_principals** de la base de données **master** . Chaque connexion au serveur a un ID unique.|  
...
|**NTDomainName** <sup>1</sup>|7|Domaine Microsoft Windows auquel appartient l'utilisateur.|  
|**NTUserName** <sup>1</sup>|6|Nom d'utilisateur Windows.|  
...
|**XactSequence**|50|Jeton servant à décrire la transaction en cours.|  
 ...
 <sup>2</sup> pour plus d’informations sur la colonne de données **ObjectType** , consultez [colonne d’événements de trace ObjectType](../event-classes/objecttype-trace-event-column.md).  
