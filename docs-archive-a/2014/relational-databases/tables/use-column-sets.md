...
# <a name="use-column-sets"></a>Utiliser des jeux de colonnes
 ...
 Pour définir un jeu de colonnes, utilisez les mots clés *<nom_jeu_colonnes>* FOR ALL_SPARSE_COLUMNS dans les instructions [CREATE TABLE](/sql/t-sql/statements/create-table-transact-sql) ou [ALTER TABLE](/sql/t-sql/statements/alter-table-transact-sql).  
...
-   Le jeu de colonnes ne peut pas être modifié. Pour modifier un jeu de colonnes, vous devez le supprimer et le recréer.  
...
-   Pour plus d’informations sur les données retournées par la fonction COLUMNS_UPDATED, consultez [Utiliser des colonnes éparses](use-sparse-columns.md).  
