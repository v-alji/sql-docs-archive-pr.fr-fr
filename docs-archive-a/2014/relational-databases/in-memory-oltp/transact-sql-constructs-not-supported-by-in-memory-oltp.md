...
|Opération|TRUNCATE TABLE|L'opération TRUNCATE n'est pas prise en charge pour les tables mémoire optimisées. Pour supprimer toutes les lignes d’une table, supprimez toutes les lignes à l’aide de `DELETE FROM` *table* ou supprimez la table, puis recréez-la.|  
