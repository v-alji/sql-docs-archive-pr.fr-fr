...
>  Utilisez la commande WITH NORECOVERY si vous envisagez de restaurer aussi une sauvegarde de base de données différentielle.  
 ...
 La syntaxe de base [RESTORE](/sql/t-sql/statements/restore-statements-transact-sql) pour la restauration d'une sauvegarde de base de données est :  
...
> [!NOTE]  
>  L’exemple commence par une instruction [ALTER DATABASE](/sql/t-sql/statements/alter-database-transact-sql) qui affecte au mode de récupération la valeur `SIMPLE`.  
