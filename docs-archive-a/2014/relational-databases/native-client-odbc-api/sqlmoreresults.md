...
>  Une instruction [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT contenant une clause COMPUTE n'est prise en charge que lors de la connexion à une version de serveur antérieure à [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)].  
 ...
 Le développeur peut déterminer les propriétés des colonnes et lignes de jeux de résultats qui sont générées par la clause COMPUTE d'un instruction [!INCLUDE[tsql](../../includes/tsql-md.md)] SELECT. Pour plus d'informations, consultez [SQLColAttribute](sqlcolattribute.md).  
