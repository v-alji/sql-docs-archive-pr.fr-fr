...
```  
 ...
 Le résultat partiel est le suivant. Étant donné que la requête spécifie ManuInstr comme nom de colonne, le code XML retourné par la `query()` méthode est encapsulé dans une `ManuInstr` balise <> comme indiqué ci-dessous :  
...
```  
 ...
 La requête suivante est similaire à l'exemple C, sauf qu'elle utilise `WITH XMLNAMESPACES` pour inclure des espaces de noms dans le résultat XML. Pour plus d’informations, consultez [Ajouter des espaces de noms aux requêtes avec WITH XMLNAMESPACES](add-namespaces-to-queries-with-with-xmlnamespaces.md).  
