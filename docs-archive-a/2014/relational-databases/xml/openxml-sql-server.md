...
#### <a name="using-the-edge-table-format"></a>Utilisation du format de table du bord
 ...
 Les tables du bord représentent au sein d'une table unique la structure à granularité fine d'un document XML. Cette structure inclut les noms des éléments et attributs, la hiérarchie du document, les espaces de noms et les instructions de traitement. Le format de table du bord vous permet d'obtenir des informations supplémentaires non présentées par le biais des métapropriétés. Pour plus d'informations sur les métapropriétés, consultez [Specify Metaproperties in OPENXML](../xml/specify-metaproperties-in-openxml.md).
