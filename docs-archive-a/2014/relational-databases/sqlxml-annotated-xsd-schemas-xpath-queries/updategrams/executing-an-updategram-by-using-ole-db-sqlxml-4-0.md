---
title: Exécution d’un mise à jour à l’aide de OLE DB (SQLXML 4,0) | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "87700871"
---
# <a name="executing-an-updategram-by-using-ole-db-sqlxml-40"></a><span data-ttu-id="18576-102">Exécution d'un code de mise à jour (updategram) à l'aide d'OLE DB (SQLXML 4.0)</span><span class="sxs-lookup"><span data-stu-id="18576-102">Executing an Updategram by Using OLE DB (SQLXML 4.0)</span></span>
  <span data-ttu-id="18576-103">Cette rubrique fournit un exemple fonctionnel de usingOLE DB pour exécuter un mise à jour.</span><span class="sxs-lookup"><span data-stu-id="18576-103">This topic provides a working sample of usingOLE DB to execute an updategram.</span></span>  
  
## <a name="using-icommandstream-to-set-an-xml-command"></a><span data-ttu-id="18576-104">Utilisation d'ICommandStream pour définir une commande XML</span><span class="sxs-lookup"><span data-stu-id="18576-104">Using ICommandStream to Set an XML Command</span></span>  
 <span data-ttu-id="18576-105">L’interface OLE DB (version 2,6 ou ultérieure) ICommandStream passe une commande en tant qu’objet de flux, plutôt qu’en tant que chaîne.</span><span class="sxs-lookup"><span data-stu-id="18576-105">The OLE DB (version 2.6 or later) ICommandStream interface passes a command as a stream object, rather than as a string.</span></span>  
  
 <span data-ttu-id="18576-106">Cette interface permet à une commande d'être encodée dans n'importe quel format pris en charge par l'analyseur XML.</span><span class="sxs-lookup"><span data-stu-id="18576-106">This interface allows a command to be in any encoding that the XML parser understands.</span></span> <span data-ttu-id="18576-107">Lorsque ICommand :: Execute est appelé, le texte de la commande est lu directement dans le flux et aucune conversion n’est requise.</span><span class="sxs-lookup"><span data-stu-id="18576-107">When ICommand::Execute is called, the command text is read out of the stream directly and no conversion is required.</span></span> <span data-ttu-id="18576-108">Par conséquent, l’exécution de commandes XML à l’aide de l’interface ICommandStream est plus efficace.</span><span class="sxs-lookup"><span data-stu-id="18576-108">Therefore, executing XML commands using ICommandStream interface is more efficient.</span></span>  
  
### <a name="setting-xml-as-a-command-using-icommandstream-and-retrieving-the-results-as-an-xml-document"></a><span data-ttu-id="18576-109">Définition de code XML en tant que commande à l'aide d'ICommandStream et récupération les résultats sous forme d'un document XML</span><span class="sxs-lookup"><span data-stu-id="18576-109">Setting XML as a Command Using ICommandStream and Retrieving the Results as an XML Document</span></span>  
 <span data-ttu-id="18576-110">L’interface ICommandStream peut être utilisée pour définir des documents XML en tant que commande et les résultats peuvent être récupérés sous la forme d’un document XML.</span><span class="sxs-lookup"><span data-stu-id="18576-110">The ICommandStream interface can be used to set XML documents as a command, and the results can be retrieved as an XML document.</span></span>  
  
#### <a name="executing-templates-with-xpath-queries"></a><span data-ttu-id="18576-111">Exécution de modèles avec des requêtes XPath</span><span class="sxs-lookup"><span data-stu-id="18576-111">Executing Templates with XPath Queries</span></span>  
 <span data-ttu-id="18576-112">Le modèle XML suivant consistant en une requête XPath est spécifié en tant que commande à l’aide de ICommandStream :</span><span class="sxs-lookup"><span data-stu-id="18576-112">The following XML template consisting of an XPath query is specified as a command using ICommandStream:</span></span>  
...
```  
 ...
 <span data-ttu-id="18576-113">La requête XPath dans le modèle est exécutée sur le schéma de mappage suivant :</span><span class="sxs-lookup"><span data-stu-id="18576-113">The XPath query in the template is executed against the following mapping schema:</span></span>  
...
```  
 ...
 <span data-ttu-id="18576-114">La requête retourne tous les éléments d'employé.</span><span class="sxs-lookup"><span data-stu-id="18576-114">The query returns all of the employee elements.</span></span> <span data-ttu-id="18576-115">Avec le mappage par défaut, l' **\<Person.Contact>** élément est mappé à la table Person. contact de la base de données AdventureWorks.</span><span class="sxs-lookup"><span data-stu-id="18576-115">With default mapping, the **\<Person.Contact>** element maps to the Person.Contact table in the AdventureWorks database.</span></span>  
  
###### <a name="to-set-xml-as-a-command-and-retrieving-result-as-an-xml-document"></a><span data-ttu-id="18576-116">Pour définir le code XML en tant que commande et récupérer les résultats sous forme d'un document XML</span><span class="sxs-lookup"><span data-stu-id="18576-116">To set XML as a command and retrieving result as an XML document</span></span>  
  
1.  <span data-ttu-id="18576-117">Initialisez et établissez une connexion à la base de données.</span><span class="sxs-lookup"><span data-stu-id="18576-117">Initialize and establish a connection to the database.</span></span>  
  
2.  <span data-ttu-id="18576-118">Obtenez l’interface ICommandStream sur ICommand.</span><span class="sxs-lookup"><span data-stu-id="18576-118">Obtain the ICommandStream interface on ICommand.</span></span>  
  
3.  <span data-ttu-id="18576-119">Définissez les propriétés de commande nécessaires.</span><span class="sxs-lookup"><span data-stu-id="18576-119">Set the necessary command properties.</span></span> <span data-ttu-id="18576-120">Dans cet exemple, la propriété SSPROP_STREAM_BASEPATH spécifique au fournisseur a pour valeur le répertoire où le schéma de mappage et les fichiers modèles sont stockés.</span><span class="sxs-lookup"><span data-stu-id="18576-120">In this example, the provider-specific property SSPROP_STREAM_BASEPATH is set to the directory where the mapping schema and the template files are stored.</span></span>  
  
4.  <span data-ttu-id="18576-121">Utilisez ICommandStream :: SetCommandStream pour spécifier le flux de commande.</span><span class="sxs-lookup"><span data-stu-id="18576-121">Use ICommandStream::SetCommandStream to specify the command stream.</span></span> <span data-ttu-id="18576-122">Dans cet exemple, le modèle XML qui est exécuté est lu à partir d'un fichier.</span><span class="sxs-lookup"><span data-stu-id="18576-122">In this example, the XML template being executed is read from a file.</span></span> <span data-ttu-id="18576-123">Cela s'avère utile lorsque vous souhaitez exécuter de grands modèles XML.</span><span class="sxs-lookup"><span data-stu-id="18576-123">This is useful when you want to execute large XML templates.</span></span>  
  
5.  <span data-ttu-id="18576-124">Exécutez la commande XML à l’aide de ICommand :: Execute, en demandant l’ID d’interface IID_ISequentialStream.</span><span class="sxs-lookup"><span data-stu-id="18576-124">Execute the XML command using ICommand::Execute, requesting the IID_ISequentialStream interface ID.</span></span>  
  
6.  <span data-ttu-id="18576-125">Traitez le résultat.</span><span class="sxs-lookup"><span data-stu-id="18576-125">Process the result.</span></span> <span data-ttu-id="18576-126">Dans cet exemple, le code XML lu à partir du flux est affiché à l'écran.</span><span class="sxs-lookup"><span data-stu-id="18576-126">In this example, the XML read from the stream is displayed on the screen.</span></span>  
...
#### <a name="passing-parameters-to-templates"></a><span data-ttu-id="18576-127">Transmission des paramètres aux modèles</span><span class="sxs-lookup"><span data-stu-id="18576-127">Passing Parameters to Templates</span></span>  
 <span data-ttu-id="18576-128">Cet exemple montre comment les valeurs de paramètre peuvent être passées aux commandes XML.</span><span class="sxs-lookup"><span data-stu-id="18576-128">This example shows how parameter values can be passed to XML commands.</span></span> <span data-ttu-id="18576-129">Ce modèle XML est spécifié en tant que commande :</span><span class="sxs-lookup"><span data-stu-id="18576-129">This XML template is specified as a command:</span></span>  
...
```  
 ...
 <span data-ttu-id="18576-130">Le modèle inclut une requête SQL.</span><span class="sxs-lookup"><span data-stu-id="18576-130">The template includes an SQL query.</span></span> <span data-ttu-id="18576-131">La requête requiert une valeur pour son paramètre ( @Title ).</span><span class="sxs-lookup"><span data-stu-id="18576-131">The query requires a value for its parameter (@Title).</span></span> <span data-ttu-id="18576-132">Si aucune valeur de paramètre n'est passée, la valeur par défaut ("Mr.") est utilisée.</span><span class="sxs-lookup"><span data-stu-id="18576-132">If no parameter value is passed, the default value ("Mr.") is used.</span></span>  
  
 <span data-ttu-id="18576-133">Pour passer des valeurs de paramètre à un modèle, le nom et la valeur du paramètre doivent être spécifiés.</span><span class="sxs-lookup"><span data-stu-id="18576-133">In passing parameter values to a template, the parameter name and value both must be specified.</span></span>  
  
 <span data-ttu-id="18576-134">Voici le code :</span><span class="sxs-lookup"><span data-stu-id="18576-134">This is the code:</span></span>  
