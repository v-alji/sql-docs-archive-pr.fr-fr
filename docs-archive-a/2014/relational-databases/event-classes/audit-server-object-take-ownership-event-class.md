...
## <a name="audit-server-object-take-ownership-event-class-data-columns"></a>Colonnes de données de la classe d'événements Audit Server Object Take Ownership  
...
|**ObjectType**|**int**|Valeur représentant le type de l'objet qui intervient dans l'événement. La valeur retournée pour cette colonne est une combinaison de la valeur correspondante dans la colonne **type** de l’affichage catalogue **sys.objects** et des valeurs répertoriées dans [ObjectType Trace Event Column](objecttype-trace-event-column.md).|28|Oui|  
