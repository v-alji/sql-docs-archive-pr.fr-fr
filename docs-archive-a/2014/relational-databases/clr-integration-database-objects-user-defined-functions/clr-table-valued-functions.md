...
# <a name="clr-table-valued-functions"></a>Fonctions table CLR
 ...
 Depuis [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)], [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] étend les fonctionnalités des fonctions table en vous permettant de définir une fonction table dans n'importe quel langage managé. Les données sont retournées à partir d'une fonction table via un objet `IEnumerable` ou `IEnumerator`.  
  
> [!NOTE]  
>  Pour les fonctions table, les colonnes du type de la table de retour ne peuvent pas inclure de colonnes timestamp ou de colonnes de type de données chaîne non-Unicode (par exemple `char`, `varchar` et `text`). La contrainte NOT NULL n'est pas prise en charge.  
 ...
 **Remarque** Une fonction table peut uniquement effectuer l’accès aux données par le biais d’une [!INCLUDE[tsql](../../includes/tsql-md.md)] requête dans la `InitMethod` méthode, et non dans la `FillRow` méthode. La méthode `InitMethod` doit être marquée avec la propriété d'attribut `SqlFunction.DataAccess.Read` si une requête [!INCLUDE[tsql](../../includes/tsql-md.md)] est effectuée.  
