---
title: Envoyer des données à une colonne FILESTREAM à l’aide de ISequentialStream liée au paramètre ICommandText (OLE DB) | Microsoft Docs
ms.custom: ''
ms.date: 03/08/2017
...
ms.locfileid: "87604429"
---
# <a name="send-data-to-a-filestream-column-using-isequentialstream-bound-to-icommandtext-parameter-ole-db"></a><span data-ttu-id="0386a-102">Envoyer des données à une colonne FILESTREAM à l'aide de l'interface ISequentialStream liée au paramètre ICommandText (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="0386a-102">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter (OLE DB)</span></span>
  <span data-ttu-id="0386a-103">Cet exemple utilise une interface ISequentialStream liée à un paramètre ICommandText pour envoyer entre 4 Mo et 4 Go de données à une colonne FILESTREAM.</span><span class="sxs-lookup"><span data-stu-id="0386a-103">This sample uses an ISequentialStream interface bound to an ICommandText parameter to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="0386a-104">Pour plus d’informations sur la fonctionnalité FileStream, consultez [prise en charge de filestream &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="0386a-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="0386a-105">Exemple</span><span class="sxs-lookup"><span data-stu-id="0386a-105">Example</span></span>  
 <span data-ttu-id="0386a-106">Avant de compiler et d'exécuter cet exemple, activez la prise en charge FILESTREAM ([Activer et configurer FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span><span class="sxs-lookup"><span data-stu-id="0386a-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="0386a-107">Assurez-vous que votre variable d'environnement INCLUDE inclut le répertoire qui contient sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="0386a-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="0386a-108">Le serveur doit disposer d’un répertoire appelé C:\DBFsa ; c'est à cet emplacement que l'exemple va créer la base de données.</span><span class="sxs-lookup"><span data-stu-id="0386a-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="0386a-109">Votre instance de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] doit disposer d'un accès en écriture à cet emplacement (par exemple, ouvrez une session en tant que compte système local).</span><span class="sxs-lookup"><span data-stu-id="0386a-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="0386a-110">Copiez la première liste de code et collez-la dans un fichier appelé ISSHelper.h.</span><span class="sxs-lookup"><span data-stu-id="0386a-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="0386a-111">Copiez la deuxième liste de code et collez-la dans un fichier appelé ISSHelper.cpp.</span><span class="sxs-lookup"><span data-stu-id="0386a-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="0386a-112">Copiez la troisième liste de code et collez-la dans un fichier appelé ICommandUpload.cpp.</span><span class="sxs-lookup"><span data-stu-id="0386a-112">Copy the third code listing and paste it into a file called ICommandUpload.cpp.</span></span>  
  
 <span data-ttu-id="0386a-113">Compilez ICommandUpload.cpp, ISSHelper.cpp, ole32.lib et oleaut32.lib.</span><span class="sxs-lookup"><span data-stu-id="0386a-113">Compile ICommandUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="0386a-114">Lorsque vous exécutez cet exemple, vous devez transmettre le nom d'un serveur, ou un serveur\nom_instance, ainsi qu'une valeur comprise entre 4 Mo (0x400001) et 4 Go (0xFFFFFFFF) indiquant la quantité de données à écrire.</span><span class="sxs-lookup"><span data-stu-id="0386a-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="0386a-115">La quatrième liste de code ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) supprime la base de données créée par cet exemple.</span><span class="sxs-lookup"><span data-stu-id="0386a-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
