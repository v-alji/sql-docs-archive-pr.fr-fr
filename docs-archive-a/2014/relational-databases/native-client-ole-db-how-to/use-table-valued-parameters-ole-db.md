---
title: Utiliser des paramètres table (OLE DB) | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87707472"
---
# <a name="use-table-valued-parameters-ole-db"></a><span data-ttu-id="1e213-102">Utiliser des paramètres table (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="1e213-102">Use Table-Valued Parameters (OLE DB)</span></span>
  <span data-ttu-id="1e213-103">Cet exemple fonctionne avec [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] ou version ultérieure.</span><span class="sxs-lookup"><span data-stu-id="1e213-103">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span> <span data-ttu-id="1e213-104">L’exemple fait ce qui suit :</span><span class="sxs-lookup"><span data-stu-id="1e213-104">This sample does the following:</span></span>  
  
-   <span data-ttu-id="1e213-105">Création de paramètres table à l’aide de la découverte dynamique via IOpenRowset::OpenRowset.</span><span class="sxs-lookup"><span data-stu-id="1e213-105">Creates table-valued parameters by using dynamic discovery though IOpenRowset::OpenRowset.</span></span>  
  
-   <span data-ttu-id="1e213-106">Envoi des lignes de paramètres table en utilisant le modèle d'extraction des données dans la classe EmployeesRowset.</span><span class="sxs-lookup"><span data-stu-id="1e213-106">Sends table-valued parameter rows by using the pull model in the EmployeesRowset class.</span></span> <span data-ttu-id="1e213-107">Dans le modèle d'extraction des données, le consommateur fournit des données au fournisseur à la demande.</span><span class="sxs-lookup"><span data-stu-id="1e213-107">In the pull model, the consumer provides data on demand to the provider.</span></span>  
  
-   <span data-ttu-id="1e213-108">Envoi des objets blob dans le cadre d'un paramètre table dans la classe CPhotograph.</span><span class="sxs-lookup"><span data-stu-id="1e213-108">Sends BLOBs as part of a table-valued parameter in the CPhotograph class.</span></span>  
  
-   <span data-ttu-id="1e213-109">Utilisation des propriétés de paramètre personnalisées à l'aide de ISSCommandWithParameters.</span><span class="sxs-lookup"><span data-stu-id="1e213-109">Uses custom parameter properties using ISSCommandWithParameters.</span></span>  
  
-   <span data-ttu-id="1e213-110">Affichage de la gestion des erreurs pour les erreurs de SQLNCLI11.</span><span class="sxs-lookup"><span data-stu-id="1e213-110">Shows error handling for SQLNCLI11 errors.</span></span>  
  
 <span data-ttu-id="1e213-111">Pour plus d’informations sur les paramètres table, consultez [paramètres table &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md).</span><span class="sxs-lookup"><span data-stu-id="1e213-111">For more information about table-valued parameters, see [Table-Valued Parameters &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="1e213-112">Exemple</span><span class="sxs-lookup"><span data-stu-id="1e213-112">Example</span></span>  
 <span data-ttu-id="1e213-113">La première liste de code ([!INCLUDE[tsql](../../includes/tsql-md.md)]) crée la base de données utilisée par l'exemple.</span><span class="sxs-lookup"><span data-stu-id="1e213-113">The first ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
  
 <span data-ttu-id="1e213-114">Placez la deuxième liste de code dans un fichier appelé stdafx.h.</span><span class="sxs-lookup"><span data-stu-id="1e213-114">Put the second code listing into a file called stdafx.h.</span></span>  
  
 <span data-ttu-id="1e213-115">Placez la troisième liste de code dans un fichier appelé OLEDBUtils.hpp.</span><span class="sxs-lookup"><span data-stu-id="1e213-115">Put the third code listing into a file called OLEDBUtils.hpp.</span></span>  
  
 <span data-ttu-id="1e213-116">Compilez avec ole32.lib oleaut32.lib et exécutez la quatrième liste de code  (C++).</span><span class="sxs-lookup"><span data-stu-id="1e213-116">Compile with ole32.lib oleaut32.lib and execute the fourth (C++) code listing.</span></span> <span data-ttu-id="1e213-117">Cette application vous permet de vous connecter à l'instance de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] par défaut de votre ordinateur.</span><span class="sxs-lookup"><span data-stu-id="1e213-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="1e213-118">Sur certains systèmes d'exploitation Windows, vous devrez remplacer (localhost) ou (local) par le nom de votre instance [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] .</span><span class="sxs-lookup"><span data-stu-id="1e213-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="1e213-119">Pour vous connecter à une instance nommée, changez la chaîne de connexion de L"(local)" en L"(local)\\\nom", où nom correspond à l’instance nommée.</span><span class="sxs-lookup"><span data-stu-id="1e213-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="1e213-120">Par défaut, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express est installé dans une instance nommée.</span><span class="sxs-lookup"><span data-stu-id="1e213-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="1e213-121">Assurez-vous que votre variable d'environnement INCLUDE inclut le répertoire qui contient sqlncli.h.</span><span class="sxs-lookup"><span data-stu-id="1e213-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="1e213-122">La cinquième liste de code ([!INCLUDE[tsql](../../includes/tsql-md.md)]) crée la base de données utilisée par l'exemple.</span><span class="sxs-lookup"><span data-stu-id="1e213-122">The fifth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing creates the database used by the sample.</span></span>  
