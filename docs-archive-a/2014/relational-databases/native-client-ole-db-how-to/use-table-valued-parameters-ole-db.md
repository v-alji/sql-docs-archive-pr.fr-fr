...
-   Envoi des lignes de paramètres table en utilisant le modèle d'extraction des données dans la classe EmployeesRowset. Dans le modèle d'extraction des données, le consommateur fournit des données au fournisseur à la demande.  
  
-   Envoi des objets blob dans le cadre d'un paramètre table dans la classe CPhotograph.  
  
-   Utilisation des propriétés de paramètre personnalisées à l'aide de ISSCommandWithParameters.  
  
-   Affichage de la gestion des erreurs pour les erreurs de SQLNCLI11.  
  
 Pour plus d’informations sur les paramètres table, consultez [paramètres table &#40;SQL Server Native Client&#41;](../native-client/features/table-valued-parameters-sql-server-native-client.md).  
  
## <a name="example"></a>Exemple  
 La première liste de code ([!INCLUDE[tsql](../../includes/tsql-md.md)]) crée la base de données utilisée par l'exemple.  
  
 Placez la deuxième liste de code dans un fichier appelé stdafx.h.  
