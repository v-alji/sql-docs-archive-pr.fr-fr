...
# <a name="prepared-execution"></a>Exécution préparée
 ...
 Certaines anciennes applications ODBC utilisaient **SQLPrepare** à chaque fois que [SQLBindParameter](../../native-client-odbc-api/sqlbindparameter.md) était utilisé. **SQLBindParameter** ne requiert pas l’utilisation de **SQLPrepare**, il peut être utilisé avec **SQLExecDirect**. Par exemple, utilisez **SQLExecDirect** avec **SQLBindParameter** pour récupérer le code de retour ou les paramètres de sortie d’une procédure stockée qui n’est exécutée qu’une seule fois. N’utilisez pas **SQLPrepare** avec **SQLBindParameter** , sauf si la même instruction est exécutée plusieurs fois.  
