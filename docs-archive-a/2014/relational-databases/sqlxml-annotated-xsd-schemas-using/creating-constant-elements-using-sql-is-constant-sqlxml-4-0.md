...
-   Création d’éléments conteneurs, tels qu’un **\<Orders>** élément qui encapsule toutes les commandes.  
 ...
 L' `sql:is-constant` annotation peut être ajoutée à un **\<complexType>** élément.  
...
### <a name="a-specifying-sqlis-constant-to-add-a-container-element"></a>R. Spécification de sql:is-constant pour ajouter un élément conteneur  
 Dans ce schéma XSD annoté, **\<CustomerOrders>** est défini en tant qu’élément constant en spécifiant l' `sql:is-constant` attribut avec la valeur 1. Par conséquent, **\<CustomerOrders>** n’est mappé à aucune table ou colonne de base de données. Cet élément constant se compose des **\<Order>** éléments enfants.  
