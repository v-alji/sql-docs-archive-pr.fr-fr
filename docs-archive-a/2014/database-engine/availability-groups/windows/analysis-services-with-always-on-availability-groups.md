...
##  <a name="checklist-use-a-secondary-replica-for-read-only-operations"></a><a name="bkmk_UseSecondary"></a> Liste de contrôle : Utiliser un réplica secondaire pour les opérations en lecture seule  
...
##  <a name="create-an-analysis-services-data-source-using-an-alwayson-availability-database"></a><a name="bkmk_ssasAODB"></a>Créer une source de données Analysis Services à l’aide d’une base de données de disponibilité AlwaysOn  
...
##  <a name="test-the-configuration"></a><a name="bkmk_test"></a>Tester la configuration  
...
7.  Vérifiez que le basculement a réussi :  
    ...
    -   Affichez le tableau de bord pour déterminer si des problèmes d'intégrité ont été détectés. Cliquez avec le bouton droit sur le groupe de disponibilité, puis sélectionnez **afficher le tableau de bord**.  
...
##  <a name="what-happens-after-a-failover-occurs"></a><a name="bkmk_whathappens"></a>Que se passe-t-il après un basculement ?  
...
##  <a name="writeback-when-using-an-alwayson-availability-database"></a><a name="bkmk_writeback"></a>Écriture différée avec une base de données de disponibilité AlwaysOn  
