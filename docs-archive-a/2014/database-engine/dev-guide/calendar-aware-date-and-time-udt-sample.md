...
3.  Dans c:\MySample, créez `CalendarAware.cs` et copiez l'exemple de code C# (ci-dessous) dans le fichier.  
  
4.  Dans c:\MySample, créez le fichier `calendars.txt` et copiez l'exemple de code C# dans le fichier.  
  
5.  Dans c:\MySample, créez le fichier `calendars.ar-SA.txt` et copiez ce qui suit dans ce fichier :  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  Dans c:\MySample, créez le fichier `calendars.ja.txt` et copiez ce qui suit dans ce fichier :  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  Dans c:\MySample, créez le fichier `calendars.``zh-CN.txt` et copiez ce qui suit dans ce fichier :  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  Dans c:\MySample, créez le fichier `build.com` et copiez ce qui suit dans ce fichier :  
...
9. Générez l'assembly satellite en exécutant la version de fichier à l'invite de commandes.  
  
10. Compilez l'exemple de code de l'invite de ligne de commande en exécutant les éléments suivants :  
...
11. Copiez le code d'installation [!INCLUDE[tsql](../../includes/tsql-md.md)] dans un fichier et enregistrez-le sous `Install.sql` dans le répertoire d'exemple.  
  
12. Si l'exemple est installé dans un répertoire autre que `C:\MySample\`, modifiez le fichier `Install.sql` comme indiqué pour pointer sur cet emplacement.  
  
13. Déployez l'assembly et la procédure stockée en exécutant  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. Copiez le script de la commande de test [!INCLUDE[tsql](../../includes/tsql-md.md)] dans un fichier et enregistrez-le sous `test.sql` dans le répertoire d'exemple.  
  
15. Exécutez le script de test avec la commande suivante  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. Copiez le script de nettoyage [!INCLUDE[tsql](../../includes/tsql-md.md)] dans un fichier et enregistrez-le sous `cleanup.sql` dans le répertoire d'exemple.  
  
17. Exécutez le script avec la commande suivante  
...
```  
 ...
 Il s'agit du fichier `calendars.txt` requis pour la définition des calendriers.  
...
```  
 ...
 Il s'agit du script d'installation [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`), qui déploie les assemblys et crée les fonctions de conversion dans la base de données ; il crée également une table d'exemples de données.  
...
```  
 ...
 Il s'agit de `test.sql`, qui teste l'exemple en exécutant les fonctions sur différents calendriers.  
...
```  
 ...
 Le code [!INCLUDE[tsql](../../includes/tsql-md.md)] suivant supprime les assemblys, les fonctions et la table de la base de données.  
