...
---
# <a name="handling-large-objects-using-clr"></a><span data-ttu-id="bd1da-102">Gestion d'objets volumineux à l'aide de CLR</span><span class="sxs-lookup"><span data-stu-id="bd1da-102">Handling Large Objects Using CLR</span></span>
  <span data-ttu-id="bd1da-103">L'exemple `HandlingLOBUsingCLR` pour SQL Server illustre le transfert d'objets LOB (Large Objects) entre [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] et un système de fichiers qui est disponible pour le serveur au moyen de procédures stockées CLR (Common Language Runtime).</span><span class="sxs-lookup"><span data-stu-id="bd1da-103">The `HandlingLOBUsingCLR` sample for SQL Server demonstrates transferring large objects (LOBs) between [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] and a file system that is available to the server by using common language runtime (CLR) stored procedures.</span></span> <span data-ttu-id="bd1da-104">Cet exemple montre comment accéder à des fichiers dans du code côté serveur et comment appeler ensuite des requêtes dynamiques et des procédures stockées à partir de procédures stockées CLR.</span><span class="sxs-lookup"><span data-stu-id="bd1da-104">This sample demonstrates how to access files in server-side code, and then invoke both dynamic queries and stored procedures from CLR-based stored procedures.</span></span> <span data-ttu-id="bd1da-105">Il montre également comment inscrire et désinscrire des méthodes et des assemblys CLR à l'aide de [!INCLUDE[tsql](../../includes/tsql-md.md)].</span><span class="sxs-lookup"><span data-stu-id="bd1da-105">It also demonstrates how to unregister and register CLR methods and assemblies by using [!INCLUDE[tsql](../../includes/tsql-md.md)].</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="bd1da-106">Prérequis</span><span class="sxs-lookup"><span data-stu-id="bd1da-106">Prerequisites</span></span>  
 <span data-ttu-id="bd1da-107">Pour créer et exécuter ce projet, les logiciels suivants doivent être installés :</span><span class="sxs-lookup"><span data-stu-id="bd1da-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="bd1da-108">ou [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="bd1da-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="bd1da-109">Vous pouvez vous procurer gratuitement [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express à partir du site Web [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [(en anglais)](https://www.microsoft.com/download/details.aspx?id=42299)</span><span class="sxs-lookup"><span data-stu-id="bd1da-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="bd1da-110">Base de données AdventureWorks qui est disponible sur le [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site Web [du Centre pour les développeurs](https://archive.codeplex.com/?p=SqlServerSamples)</span><span class="sxs-lookup"><span data-stu-id="bd1da-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="bd1da-111">Le Kit de développement logiciel .NET Framework SDK 2.0 ou version ultérieure, ou Microsoft Visual Studio 2005 ou version ultérieure.</span><span class="sxs-lookup"><span data-stu-id="bd1da-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="bd1da-112">Vous pouvez vous procurer gratuitement le Kit de développement logiciel .NET Framework SDK.</span><span class="sxs-lookup"><span data-stu-id="bd1da-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="bd1da-113">De plus, les conditions suivantes doivent être réunies :</span><span class="sxs-lookup"><span data-stu-id="bd1da-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="bd1da-114">L'intégration du CLR doit être activée sur l'instance [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que vous utilisez.</span><span class="sxs-lookup"><span data-stu-id="bd1da-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="bd1da-115">Pour activer l'intégration du CLR, effectuez les étapes suivantes :</span><span class="sxs-lookup"><span data-stu-id="bd1da-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="bd1da-116">Activation de l'intégration du CLR</span><span class="sxs-lookup"><span data-stu-id="bd1da-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="bd1da-117">Exécutez les commandes [!INCLUDE[tsql](../../includes/tsql-md.md)] suivantes :</span><span class="sxs-lookup"><span data-stu-id="bd1da-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="bd1da-118">Pour activer l'intégration du CLR, vous devez disposer de l'autorisation de niveau serveur `ALTER SETTINGS` qui est attribuée implicitement aux membres des rôles serveur fixes `sysadmin` et `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="bd1da-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="bd1da-119">La base de données AdventureWorks doit être installée sur l'instance [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que vous utilisez.</span><span class="sxs-lookup"><span data-stu-id="bd1da-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="bd1da-120">Si vous n’êtes pas administrateur de l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance que vous utilisez, vous devez demander à un administrateur de vous accorder l’autorisation **CreateAssembly** pour terminer l’installation.</span><span class="sxs-lookup"><span data-stu-id="bd1da-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="bd1da-121">Génération de l'exemple</span><span class="sxs-lookup"><span data-stu-id="bd1da-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="bd1da-122">Créez et exécutez l'exemple à l'aide des instructions suivantes :</span><span class="sxs-lookup"><span data-stu-id="bd1da-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="bd1da-123">Ouvrez une invite de commandes Visual Studio ou .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="bd1da-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="bd1da-124">Si nécessaire, créez un répertoire pour votre exemple.</span><span class="sxs-lookup"><span data-stu-id="bd1da-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="bd1da-125">Pour cet exemple, nous utiliserons C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="bd1da-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="bd1da-126">Puisque cet exemple requiert un assembly signé, créez une clé asymétrique en tapant la commande :</span><span class="sxs-lookup"><span data-stu-id="bd1da-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="bd1da-127">Exemple de code</span><span class="sxs-lookup"><span data-stu-id="bd1da-127">Sample Code</span></span>  
 <span data-ttu-id="bd1da-128">Voici les listes de code pour cet exemple.</span><span class="sxs-lookup"><span data-stu-id="bd1da-128">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="bd1da-129">C#</span><span class="sxs-lookup"><span data-stu-id="bd1da-129">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="bd1da-130">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="bd1da-130">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="bd1da-131">Il s'agit du script d'installation [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`), qui déploie l'assembly et crée les procédures stockées et les objets de sécurité requis par cet exemple.</span><span class="sxs-lookup"><span data-stu-id="bd1da-131">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the stored procedures and security objects required by this example.</span></span>  
...
```  
 ...
 <span data-ttu-id="bd1da-132">Le code suivant [!INCLUDE[tsql](../../includes/tsql-md.md)] ( `test.sql` ) teste l’exemple en exerçant les procédures stockées.</span><span class="sxs-lookup"><span data-stu-id="bd1da-132">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] (`test.sql`) tests the example by exercising the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="bd1da-133">Le code [!INCLUDE[tsql](../../includes/tsql-md.md)] suivant supprime l'assembly, le compte de connexion, les procédures stockées et les clés de la base de données.</span><span class="sxs-lookup"><span data-stu-id="bd1da-133">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly, login, key and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="bd1da-134">Voir aussi</span><span class="sxs-lookup"><span data-stu-id="bd1da-134">See Also</span></span>  
 [<span data-ttu-id="bd1da-135">Scénarios et exemples d’utilisation pour l’intégration du CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="bd1da-135">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
