...
3.  Dans c:\MySample, créez `StringUtils.vb` (pour l'exemple Visual Basic) ou `StringUtils.cs` (pour l'exemple C#) et copiez l'exemple de code Visual Basic ou  C# approprié (ci-dessous) dans le fichier.  
...
7.  Déployez l'assembly et la procédure stockée en exécutant  
    ...
    -   Copiez le script de la commande de test [!INCLUDE[tsql](../../includes/tsql-md.md)] dans un fichier et enregistrez-le sous `test.sql` dans le répertoire d'exemple.  
