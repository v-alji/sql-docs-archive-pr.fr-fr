...
3.  Dans c:\MySample, créez `ComplexNumber.vb` (pour l'exemple Visual Basic) ou `ComplexNumber.cs` (pour l'exemple C#) et copiez l'exemple de code Visual Basic ou  C# approprié (ci-dessous) dans le fichier.  
...
6.  Copiez le code d'installation [!INCLUDE[tsql](../../includes/tsql-md.md)] dans un fichier et enregistrez-le sous `Install.sql` dans le répertoire d'exemple.  
  
7.  Si l'exemple est installé dans un répertoire autre que `C:\MySample\`, modifiez le fichier `Install.sql` comme indiqué pour pointer sur cet emplacement.  
  
8.  Déployez l'assembly et la procédure stockée en exécutant  
...
11. Pour exécuter le programme d'exemple client (C# uniquement) :  
  
    -   Dans c:\MySample, créez `Program.cs` et copiez le code source dans le fichier.  
  
    -   Effectue`: Csc /reference:ComplexNumber.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.Data.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.XML.dll /out:Client.exe /target:exe Program.cs`  
...
```  
 ...
 Il s'agit du script d'installation [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`), qui déploie l'assembly et crée l'UDT dans la base de données.  
...
```  
 ...
 Il s'agit de `test.sql`, qui teste l'exemple en exerçant le type.  
...
```  
 ...
 Le code [!INCLUDE[tsql](../../includes/tsql-md.md)] suivant supprime l'assembly et le type de la base de données.  
