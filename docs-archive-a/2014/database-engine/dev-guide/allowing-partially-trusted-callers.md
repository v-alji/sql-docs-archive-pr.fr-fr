---
title: Autorisation des appelants de confiance partielle | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87603001"
---
# <a name="allowing-partially-trusted-callers"></a><span data-ttu-id="658fb-102">Autorisation d'appelants partiellement approuvés</span><span class="sxs-lookup"><span data-stu-id="658fb-102">Allowing Partially Trusted Callers</span></span>
  <span data-ttu-id="658fb-103">Le partage de bibliothèques de code est un scénario courant avec l'intégration du Common Language Runtime (CLR), dans lequel un assembly qui contient un type défini par l'utilisateur, une procédure stockée, une fonction définie par l'utilisateur, un agrégat défini par l'utilisateur, un déclencheur ou une classe utilitaire est souvent accédé par un autre assembly ou application.</span><span class="sxs-lookup"><span data-stu-id="658fb-103">Sharing code libraries is a common scenario with common language runtime (CLR) integration, where an assembly containing a user-defined type, stored procedure, user-defined function, user-defined aggregate, trigger, or utility class is often accessed by another assembly or application.</span></span> <span data-ttu-id="658fb-104">Les bibliothèques de code qui doivent être partagées par plusieurs applications doivent être signées avec un nom fort.</span><span class="sxs-lookup"><span data-stu-id="658fb-104">Code libraries that are to be shared by multiple applications must be signed with a strong name.</span></span>  
  
 <span data-ttu-id="658fb-105">Seules les applications approuvées complètement par le système de sécurité d'accès du code au moment de l'exécution sont autorisées à accéder à un assembly de code managé partagé qui n'est pas marqué explicitement avec l'attribut `System.Security.AllowPartiallyTrustedCallers`.</span><span class="sxs-lookup"><span data-stu-id="658fb-105">Only applications that are fully trusted by the runtime code access security system are allowed to access a shared managed code assembly that is not explicitly marked with the `System.Security.AllowPartiallyTrustedCallers` attribute.</span></span> <span data-ttu-id="658fb-106">Un assembly partiellement approuvé (assembly inscrit dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] avec le jeu d'autorisations `SAFE` ou `EXTERNAL_ACCESS`) qui tente d'accéder à un assembly signé avec nom fort sans cet attribut provoque la levée d'une exception `System.Security.SecurityException`.</span><span class="sxs-lookup"><span data-stu-id="658fb-106">A partially trusted assembly (one that is registered in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] with the `SAFE` or `EXTERNAL_ACCESS` permission set) that attempts to access a strong-name signed assembly without this attribute causes a `System.Security.SecurityException` to be thrown.</span></span> <span data-ttu-id="658fb-107">Le message d'erreur qui s'affiche est semblable au suivant :</span><span class="sxs-lookup"><span data-stu-id="658fb-107">The error message you see is similar to the following:</span></span>  
...
```  
 ...
 <span data-ttu-id="658fb-108">Nous recommandons que tous les assemblys inscrits dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], sauf ceux ajoutés au Global Assembly Cache, soient marqués avec l'attribut `AllowPartiallyTrustedCallers` afin que les assemblys chargés par [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] puissent accéder les uns aux autres.</span><span class="sxs-lookup"><span data-stu-id="658fb-108">We recommend that all assemblies registered in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], except those assemblies added to the global assembly cache, be marked with the `AllowPartiallyTrustedCallers` attribute so that assemblies loaded by [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] can access each other.</span></span> <span data-ttu-id="658fb-109">Les assemblys qui doivent être ajoutés au Global Assembly Cache doivent être examinés minutieusement afin de vérifier leur sécurité avant l'ajout de l'attribut `AllowPartiallyTrustedCallers`, car ces assemblys seront ensuite accessibles aux appelants partiellement approuvés à partir de contextes inattendus.</span><span class="sxs-lookup"><span data-stu-id="658fb-109">Assemblies that are to be added to the global assembly cache should be thoroughly reviewed for safety before adding the `AllowPartiallyTrustedCallers` attribute, as the assembly would then be available to partially trusted callers from unexpected contexts.</span></span> <span data-ttu-id="658fb-110">Un assembly ne doit pas être validé comme approuvé complètement (inscrit avec le jeu d'autorisations `UNSAFE` dans [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]).</span><span class="sxs-lookup"><span data-stu-id="658fb-110">An assembly should not be made fully trusted (registered with the `UNSAFE` permission set in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]).</span></span>  
  
 <span data-ttu-id="658fb-111">Pour plus d'informations, consultez la section « Using Libraries from Partially Trusted Code » dans le kit de développement logiciel (SDK) .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="658fb-111">For more information, see the "Using Libraries from Partially Trusted Code" section in the .NET Framework software development kit.</span></span>  
  
## <a name="example"></a><span data-ttu-id="658fb-112">Exemple</span><span class="sxs-lookup"><span data-stu-id="658fb-112">Example</span></span>  
  
### <a name="description"></a><span data-ttu-id="658fb-113">Description</span><span class="sxs-lookup"><span data-stu-id="658fb-113">Description</span></span>  
 <span data-ttu-id="658fb-114">Supposez une classe utilitaire qui serait utile pour de nombreuses applications d'intégration du CLR côté serveur.</span><span class="sxs-lookup"><span data-stu-id="658fb-114">Suppose there is a utility class that would be useful for many server-side CLR integration applications.</span></span> <span data-ttu-id="658fb-115">Par exemple, il peut s'agir d'une classe qui représente les résultats de l'appel d'une requête.</span><span class="sxs-lookup"><span data-stu-id="658fb-115">For example, it might be a class that represents the results of invoking a query.</span></span> <span data-ttu-id="658fb-116">Pour activer le partage de ce composant, cette classe utilitaire est placée dans un assembly séparé.</span><span class="sxs-lookup"><span data-stu-id="658fb-116">To enable sharing of this component, this utility class is placed in a separate assembly.</span></span> <span data-ttu-id="658fb-117">Ensuite, cet assembly est référencé à partir de différents autres assemblys qui contiennent des objets d'intégration CLR.</span><span class="sxs-lookup"><span data-stu-id="658fb-117">Then that assembly is referenced from various other assemblies that contain CLR integration objects.</span></span> <span data-ttu-id="658fb-118">Cette classe utilitaire étant utilisée dans de nombreuses applications serveur différentes, elle est examinée avec soin et les éventuels problèmes de sécurité sont résolus.</span><span class="sxs-lookup"><span data-stu-id="658fb-118">Because this utility class is used in many different server applications, it is reviewed carefully and any security issues are resolved.</span></span> <span data-ttu-id="658fb-119">L'attribut `AllowPartiallyTrustedCallers` est ensuite appliqué à l'assembly qui contient la classe utilitaire, afin que les objets d'intégration CLR contenus dans les assemblys marqués avec le jeu d'autorisations `SAFE` ou `EXTERNAL_ACCESS` puissent utiliser la classe utilitaire et les méthodes, bien qu'elles soient dans un assembly distinct.</span><span class="sxs-lookup"><span data-stu-id="658fb-119">The `AllowPartiallyTrustedCallers` attribute is then applied to the assembly that contains the utility class, so that CLR integration objects contained in assemblies marked with `SAFE` or `EXTERNAL_ACCESS` permission sets can use the utility class and methods, even though they are in a separate assembly.</span></span>  
  
 <span data-ttu-id="658fb-120">Il est parfois utile d'être en mesure d'exécuter des commandes tout en lisant les résultats d'une requête, sans ouvrir de nouvelle connexion ni charger tous les résultats en mémoire.</span><span class="sxs-lookup"><span data-stu-id="658fb-120">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="658fb-121">La fonctionnalité MARS (Multiple Active Result Set) d'ADO.NET 2.0 est une technologie qui vous permet de le faire.</span><span class="sxs-lookup"><span data-stu-id="658fb-121">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="658fb-122">À l'heure actuelle, cette technologie n'est pas implémentée pour le fournisseur de processus interne employé dans la programmation côté serveur.</span><span class="sxs-lookup"><span data-stu-id="658fb-122">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="658fb-123">Pour contourner cette limitation, il est possible d'utiliser des curseurs côté serveur.</span><span class="sxs-lookup"><span data-stu-id="658fb-123">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="658fb-124">Cet exemple montre comment utiliser les curseurs côté serveur afin de pallier l'absence de prise en charge de MARS dans la programmation côté serveur.</span><span class="sxs-lookup"><span data-stu-id="658fb-124">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
 <span data-ttu-id="658fb-125">L'utilisation de curseurs côté serveur est coûteuse en termes de ressources du serveur et peut parfois empêcher l'optimiseur de requête de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] d'améliorer les performances de vos requêtes.</span><span class="sxs-lookup"><span data-stu-id="658fb-125">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] from enhancing the performance of your queries.</span></span> <span data-ttu-id="658fb-126">Par conséquent, il se peut que vous ayez intérêt à récrire votre code afin d'utiliser des jointures dans tous les cas où il est possible de le faire.</span><span class="sxs-lookup"><span data-stu-id="658fb-126">Therefore, you might consider rewriting your code to use JOINs whenever possible.</span></span>  
  
 <span data-ttu-id="658fb-127">L'API pour cette classe est semblable à un lecteur de données, excepté que vous pouvez vous déplacer en avant ou en arrière dans le jeu de résultats et que d'autres commandes peuvent être exécutées sur la connexion même tandis que vous ouvrez le jeu de résultats.</span><span class="sxs-lookup"><span data-stu-id="658fb-127">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.</span></span>  
  
 <span data-ttu-id="658fb-128">Cette implémentation est hautement simplifiée afin de faciliter la compréhension de l'exemple.</span><span class="sxs-lookup"><span data-stu-id="658fb-128">This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="658fb-129">Une mise en œuvre plus efficace consisterait à extraire plusieurs lignes pour éviter un parcours complet de la base de données pour chaque ligne extraite.</span><span class="sxs-lookup"><span data-stu-id="658fb-129">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span>  
  
 <span data-ttu-id="658fb-130">L'utilisation de cette classe présente un avantage très important pour la programmation côté serveur dans la mesure où elle aboutit à un encombrement mémoire nettement inférieur à celui occasionné par le remplissage d'un dataset avec tous les résultats d'une requête.</span><span class="sxs-lookup"><span data-stu-id="658fb-130">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span>  
  
 <span data-ttu-id="658fb-131">Cet exemple démontre également l'utilisation de l'attribut d'autorisation des appelants partiellement approuvés visant à indiquer que l'assembly ResultSet est une bibliothèque que vous pouvez appeler en toute sécurité à partir d'autres assemblys.</span><span class="sxs-lookup"><span data-stu-id="658fb-131">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="658fb-132">Cette approche est un peu plus complexe mais beaucoup plus sûre que d'inscrire l'assembly appelant avec l'autorisation unsafe.</span><span class="sxs-lookup"><span data-stu-id="658fb-132">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="658fb-133">Elle est plus sûre parce qu'en inscrivant l'assembly appelant en tant que safe, vous affectez de manière limitée les ressources en dehors du serveur et éviter d'endommager l'intégrité de ce dernier.</span><span class="sxs-lookup"><span data-stu-id="658fb-133">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
 <span data-ttu-id="658fb-134">Les instructions de version pour cet exemple supposent que les fichiers de code source se trouvent dans un répertoire appelé c:\samples.</span><span class="sxs-lookup"><span data-stu-id="658fb-134">The build instructions for this sample assume that the source code files are in a directory called c:\samples.</span></span>  <span data-ttu-id="658fb-135">Si vous utilisez un autre répertoire, vous devrez modifier les scripts [!INCLUDE[tsql](../../includes/tsql-md.md)].</span><span class="sxs-lookup"><span data-stu-id="658fb-135">If you use another directory, you will have to modify the [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts.</span></span> <span data-ttu-id="658fb-136">Les [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts requièrent également la base de données AdventureWorks.</span><span class="sxs-lookup"><span data-stu-id="658fb-136">The [!INCLUDE[tsql](../../includes/tsql-md.md)] scripts also requires the AdventureWorks database.</span></span> <span data-ttu-id="658fb-137">Vous pouvez télécharger l’exemple de base de données AdventureWorks à partir de la Microsoft SQL Server page d’hébergement [exemples et projets](https://go.microsoft.com/fwlink/?LinkID=85384) de la communauté.</span><span class="sxs-lookup"><span data-stu-id="658fb-137">You can download the AdventureWorks sample database from the [Microsoft SQL Server Samples and Community Projects](https://go.microsoft.com/fwlink/?LinkID=85384) home page.</span></span>  
  
 <span data-ttu-id="658fb-138">Pour générer et exécuter l'exemple, collez la première liste de code dans un fichier appelé ResultSet.cs et compilez avec csc /target:library ResultSet.cs.</span><span class="sxs-lookup"><span data-stu-id="658fb-138">To build and run the sample, paste the first code listing into a file called ResultSet.cs and compile with csc /target:library ResultSet.cs.</span></span>  
  
 <span data-ttu-id="658fb-139">Collez la deuxième liste de code dans un fichier appelé TestResultSet.cs et compilez avec csc /target:library /reference:ResultSet.dll TestResultSet.cs.</span><span class="sxs-lookup"><span data-stu-id="658fb-139">Paste the second code listing into a file called TestResultSet.cs and compile with csc /target:library /reference:ResultSet.dll TestResultSet.cs.</span></span>  
  
 <span data-ttu-id="658fb-140">Collez la troisième liste de code dans un fichier appelé InstallCS.sql et compilez avec sqlcmd -E -I -i InstallCS.sql.</span><span class="sxs-lookup"><span data-stu-id="658fb-140">Paste the third code listing into a file called InstallCS.sql and compile with sqlcmd -E -I -i InstallCS.sql.</span></span>  
  
 <span data-ttu-id="658fb-141">Collez la quatrième liste de code dans un fichier appelé test.sql et compilez avec sqlcmd -E -I -i test.sql.</span><span class="sxs-lookup"><span data-stu-id="658fb-141">Paste the fourth code listing into a file called test.sql  and compile with sqlcmd -E -I -i test.sql.</span></span>  
  
 <span data-ttu-id="658fb-142">Collez la cinquième liste de code dans un fichier appelé Cleanup.sql et compilez avec sqlcmd -E -I -i Cleanup.sql.</span><span class="sxs-lookup"><span data-stu-id="658fb-142">Paste the fifth code listing into a file called Cleanup.sql  and compile with sqlcmd -E -I -i Cleanup.sql.</span></span>  
  
### <a name="code"></a><span data-ttu-id="658fb-143">Code</span><span class="sxs-lookup"><span data-stu-id="658fb-143">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="658fb-144">Code</span><span class="sxs-lookup"><span data-stu-id="658fb-144">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="658fb-145">Code</span><span class="sxs-lookup"><span data-stu-id="658fb-145">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="658fb-146">Code</span><span class="sxs-lookup"><span data-stu-id="658fb-146">Code</span></span>  
...
### <a name="code"></a><span data-ttu-id="658fb-147">Code</span><span class="sxs-lookup"><span data-stu-id="658fb-147">Code</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="658fb-148">Voir aussi</span><span class="sxs-lookup"><span data-stu-id="658fb-148">See Also</span></span>  
 [<span data-ttu-id="658fb-149">Sécurité de l'intégration du CLR</span><span class="sxs-lookup"><span data-stu-id="658fb-149">CLR Integration Security</span></span>](../../relational-databases/clr-integration/security/clr-integration-security.md)  
