---
title: Représentation de l’indicateur de performance clé (tabulaire) | Microsoft Docs
ms.custom: ''
ms.date: 04/27/2017
...
ms.locfileid: "87706815"
---
# <a name="key-performance-indicator-representation-tabular"></a><span data-ttu-id="82603-102">Représentation (tabulaire) d'un indicateur de performance clé</span><span class="sxs-lookup"><span data-stu-id="82603-102">Key Performance Indicator Representation (Tabular)</span></span>
  <span data-ttu-id="82603-103">Un KPI évalue la performance d'une valeur, définie par une mesure de base, par rapport à une valeur cible.</span><span class="sxs-lookup"><span data-stu-id="82603-103">A KPI is used to gauge performance of a value, defined by a Base measure, against a Target value</span></span>  
  
## <a name="key-performance-indicator-representation"></a><span data-ttu-id="82603-104">Représentation d'un indicateur de performance clé</span><span class="sxs-lookup"><span data-stu-id="82603-104">Key Performance Indicator Representation</span></span>  
 <span data-ttu-id="82603-105">Dans les modèles d’objet tabulaires, un indicateur de performance clé-KPI-est une mesure avec des informations supplémentaires pour que l’application cliente l’affiche graphiquement.</span><span class="sxs-lookup"><span data-stu-id="82603-105">In tabular object models a key performance indicator -kpi- is a measure with additional information for the client application to display it graphically.</span></span> <span data-ttu-id="82603-106">Un KPI comprend généralement des informations sur l'objectif visé, l'état de la mesure par rapport à l'objectif et des informations destinées à l'outil client pour afficher graphiquement l'état.</span><span class="sxs-lookup"><span data-stu-id="82603-106">A kpi usually has information about a goal to be obtained, the status of the measure compared to a the goal, and information for the client tool on how to display graphically the status.</span></span>  
  
### <a name="key-performance-indicator-in-amo"></a><span data-ttu-id="82603-107">Indicateur de performance clé dans AMO</span><span class="sxs-lookup"><span data-stu-id="82603-107">Key Performance Indicator in AMO</span></span>  
 <span data-ttu-id="82603-108">Lorsque vous utilisez AMO pour gérer un KPI de modèle tabulaire il n'y a pas de correspondance d'objet un-à-un, l'objet AMO <xref:Microsoft.AnalysisServices.Kpi> n'est pas utilisée à cet effet ; dans AMO, pour les modèles tabulaires, un KPI est représenté par la série d'objets créés dans un des éléments de la collection <xref:Microsoft.AnalysisServices.MdxScript.Commands%2A> et de la collection <xref:Microsoft.AnalysisServices.MdxScript.CalculationProperties%2A>.</span><span class="sxs-lookup"><span data-stu-id="82603-108">When using AMO to manage a Tabular Model kpi there is no one-to-one object match for a kpi in AMO, the AMO <xref:Microsoft.AnalysisServices.Kpi>object is not used for this purpose; in AMO, for tabular models, a kpi is represented the by a series of objects created in one of the elements in the <xref:Microsoft.AnalysisServices.MdxScript.Commands%2A> collection and in the <xref:Microsoft.AnalysisServices.MdxScript.CalculationProperties%2A>.</span></span>  
  
 <span data-ttu-id="82603-109">L'extrait de code suivant montre comment créer une définition de KPI parmi les nombreuses possibles.</span><span class="sxs-lookup"><span data-stu-id="82603-109">The following code snippet shows how to create one of many possible kpi definitions.</span></span>  
...
## <a name="amo2tabular-sample"></a><span data-ttu-id="82603-110">Exemple AMO2Tabular</span><span class="sxs-lookup"><span data-stu-id="82603-110">AMO2Tabular sample</span></span>  
 <span data-ttu-id="82603-111">Pour savoir comment utiliser AMO pour créer et manipuler des représentations d'indicateur de performance clé, consultez le code source de l'exemple Objets AMO vers objets tabulaires ; plus précisément, archivez le fichier source suivant : AddKPIs.cs.</span><span class="sxs-lookup"><span data-stu-id="82603-111">To have an understanding on how to use AMO to create and manipulate Key Performance Indicator representations see the source code of the AMO to Tabular sample; specifically check in the following source file: AddKPIs.cs.</span></span> <span data-ttu-id="82603-112">L'exemple est disponible sur Codeplex.</span><span class="sxs-lookup"><span data-stu-id="82603-112">The sample is available at Codeplex.</span></span> <span data-ttu-id="82603-113">Remarque importante à propos du code : le code est fourni uniquement comme un support aux concepts logiques expliqués ici et ne doit pas être utilisé dans un environnement de production, ni à des fins autres que pédagogiques.</span><span class="sxs-lookup"><span data-stu-id="82603-113">An important note about the code: the code is provided only as a support to the logical concepts explained here and should not be used in a production environment; nor should it be used for other purpose other than the pedagogical one.</span></span>  
