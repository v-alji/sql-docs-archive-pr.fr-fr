---
title: Représentation de la relation (tabulaire) | Microsoft Docs
ms.custom: ''
ms.date: 01/19/2019
...
ms.locfileid: "87698568"
---
# <a name="relationship-representation-tabular"></a><span data-ttu-id="239b6-102">Représentation (tabulaire) d'une relation</span><span class="sxs-lookup"><span data-stu-id="239b6-102">Relationship Representation (Tabular)</span></span>
  <span data-ttu-id="239b6-103">Une relation est une connexion entre deux tables de données.</span><span class="sxs-lookup"><span data-stu-id="239b6-103">A relationship is a connection between two tables of data.</span></span> <span data-ttu-id="239b6-104">La relation établit la façon dont les données des deux tables doivent être mises en corrélation.</span><span class="sxs-lookup"><span data-stu-id="239b6-104">The relationship establishes how the data in the two tables should be correlated.</span></span>  
  
 <span data-ttu-id="239b6-105">Consultez [Relationship Representation (Tabular)](relationship-representation-tabular.md) pour une explication détaillée sur la façon de créer et manipuler la représentation d'une relation.</span><span class="sxs-lookup"><span data-stu-id="239b6-105">See [Relationship Representation (Tabular)](relationship-representation-tabular.md) for a detailed explanation on how to create and manipulate the relationship representation.</span></span>  
  
## <a name="relationship-representation"></a><span data-ttu-id="239b6-106">Représentation d'une relation</span><span class="sxs-lookup"><span data-stu-id="239b6-106">Relationship Representation</span></span>  
 <span data-ttu-id="239b6-107">Dans les modèles tabulaires, plusieurs relations peuvent être définies entre deux tables.</span><span class="sxs-lookup"><span data-stu-id="239b6-107">In tabular models, multiple relationships can be defined between two tables.</span></span> <span data-ttu-id="239b6-108">Lorsque plusieurs relations entre deux tables sont définies, une seule peut être définie en tant que relation par défaut pour le modèle et est nommée comme relation active ; toutes les autres relations sont nommées comme inactives.</span><span class="sxs-lookup"><span data-stu-id="239b6-108">When multiple relationships, between two tables, are defined only one can be defined as the default relationship for the model and it is named as the Active relationship; all other relationships are named as Inactive.</span></span>  
  
### <a name="relationship-in-amo"></a><span data-ttu-id="239b6-109">Relation dans AMO</span><span class="sxs-lookup"><span data-stu-id="239b6-109">Relationship in AMO</span></span>  
 <span data-ttu-id="239b6-110">En termes d'objets AMO, toutes les relations inactives ont une représentation de relation de mappage un-à-un avec <xref:Microsoft.AnalysisServices.Relationship> et aucun autre objet AMO principal n'est requis ; pour la relation active, d'autres exigences existent et un mappage à <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension> est également requis.</span><span class="sxs-lookup"><span data-stu-id="239b6-110">In terms of AMO objects all inactive relationships have a representation of a one to one mapping relationship with <xref:Microsoft.AnalysisServices.Relationship> and no other main AMO objects are required; for the active relationship other requirements exist and a mapping to the <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension> is also required.</span></span>  
  
 <span data-ttu-id="239b6-111">Les extraits de code suivants montrent comment créer une relation dans les modèles tabulaires, comment activer une relation et comment définir une clé primaire dans une table (autre que « RowNumber »).</span><span class="sxs-lookup"><span data-stu-id="239b6-111">The following code snippets show how to create a relationship in Tabular models, how to activate a relationship and how to define a primary key in a table (other than "RowNumber").</span></span> <span data-ttu-id="239b6-112">Pour créer une relation active, une clé primaire doit être définie dans la table de clé primaire (PKTableName) de la relation (le côté « un » de la relation). L'exemple ci-après crée la clé primaire sur PKColumnName si aucune clé primaire n'est définie dans cette colonne.</span><span class="sxs-lookup"><span data-stu-id="239b6-112">To create an active relationship a primary key need to be defined in the primary key table - PKTableName - of the relationship (the one side of the relationship), the sample shown here creates the primary key on the PKColumnName if no primary key is defined in this column.</span></span> <span data-ttu-id="239b6-113">Les relations inactives peuvent être créées sans clé primaire sur la colonne de clé primaire.</span><span class="sxs-lookup"><span data-stu-id="239b6-113">Inactive relationships can be created without the need of having a primary key on the primary key column.</span></span>  
...
## <a name="amo2tabular-sample"></a><span data-ttu-id="239b6-114">Exemple AMO2Tabular</span><span class="sxs-lookup"><span data-stu-id="239b6-114">AMO2Tabular sample</span></span>  
 <span data-ttu-id="239b6-115">Toutefois, pour connaître l'utilisation d’AMO pour créer et manipuler des représentations de relation, consultez le code source de l'exemple Objets AMO vers objets tabulaires.</span><span class="sxs-lookup"><span data-stu-id="239b6-115">However, to have an understanding on how to use AMO to create and manipulate relationship representations see the source code of the AMO to Tabular sample.</span></span> <span data-ttu-id="239b6-116">L'exemple est disponible sur Codeplex.</span><span class="sxs-lookup"><span data-stu-id="239b6-116">The sample is available at Codeplex.</span></span> <span data-ttu-id="239b6-117">Remarque importante à propos du code : le code est fourni uniquement comme un support aux concepts logiques expliqués ici et ne doit pas être utilisé dans un environnement de production, ni à des fins autres que pédagogiques.</span><span class="sxs-lookup"><span data-stu-id="239b6-117">An important note about the code: the code is provided only as a support to the logical concepts explained here and should not be used in a production environment; nor should it be used for other purpose other than the pedagogical one.</span></span>  
