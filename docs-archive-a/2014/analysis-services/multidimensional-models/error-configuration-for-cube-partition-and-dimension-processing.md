...
##  <a name="error-configuration-properties"></a><a name="bkmk_props"></a>Propriétés de configuration d’erreur  
...
|**Propriété**|**Valeurs**|  
|------------------|----------------|  
|`KeyErrorAction`|Il s'agit de l'action entreprise par le serveur lorsqu'une erreur `KeyNotFound` se produit. Les réponses valides à cette erreur sont `ConvertToUnknown` ou `DiscardRecord`.|  
