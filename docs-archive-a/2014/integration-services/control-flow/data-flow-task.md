...
# <a name="data-flow-task"></a>tâche de flux de données
 ...
 Pour insérer des données en bloc dans une base de données [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] à partir de fichiers texte, vous pouvez utiliser la tâche d'insertion en bloc plutôt qu'une tâche de flux de données et un flux de données. Cependant, la tâche d'insertion en bloc ne peut pas transformer des données. Pour plus d’informations, consultez [Tâche d’insertion en bloc](bulk-insert-task.md).  
