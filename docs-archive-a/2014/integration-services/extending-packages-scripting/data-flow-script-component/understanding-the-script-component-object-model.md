...
-   Substituez **\<inputbuffer>_ProcessInput** uniquement si vous devez effectuer une autre opération pendant que vous parcourez les lignes d’entrée, (Par exemple, vous devez tester pour que `EndOfRowset` effectue une autre action une fois que toutes les lignes ont été traitées.) Appelez ** \<inputbuffer> _ProcessInputRow** pour effectuer le traitement de ligne.
