...
##  <a name="package"></a><a name="Package"></a> Package  
 ...
 `PackageType`Définie par la propriété à l’aide des valeurs de l' `DTSPackageType` énumération.  
...
|SQLDBMaint|6|  
 ...
 `CheckpointUsage`Définie par la propriété à l’aide des valeurs de l' `DTSCheckpointUsage` énumération.  
...
|Toujours|2|  
 ...
 `PackagePriorityClass`Définie par la propriété à l’aide des valeurs de l' `DTSPriorityClass` énumération.  
...
|Idle|4|  
 ...
 `ProtectionLevel`Définie par la propriété à l’aide des valeurs de l' `DTSProtectionLevel` énumération.  
...
##  <a name="precedence-constraints"></a><a name="PrecedenceConstraints"></a> Contraintes de précédence  
 `EvalOp`Définie par la propriété à l’aide des valeurs de l' `DTSPrecedenceEvalOp` énumération.  
...
|ExpressionOrConstraint|4|  
 ...
 `Value`Définie par la propriété à l’aide des valeurs de l' `DTSExecResult` énumération.  
...
### <a name="foreach-ado-enumerator"></a>Énumérateur Foreach ADO  
 `Type`Définie par la propriété à l’aide des valeurs de l' `ADOEnumerationType` énumération.  
...
### <a name="foreach-nodelist-enumerator"></a>Énumérateur Foreach Nodelist  
 `SourceDocumentType``InnerXPathStringSourceType`Propriétés, et **OuterXPathStringSourceType** -définies à l’aide des valeurs de l' `SourceType` énumération.  
...
|DirectInput|2|  
 ...
 `EnumerationType`Définie par la propriété à l’aide des valeurs de l' `EnumerationType` énumération.  
...
|ElementCollection|3|  
 ...
 `InnerElementType`Définie par la propriété à l’aide des valeurs de l' `InnerElementType` énumération.  
...
### <a name="analysis-services-execute-ddl-task"></a>Tâche DDL d'exécution de SQL Server Analysis Services  
 `SourceType`Définie par la propriété à l’aide des valeurs de l' `DDLSourceType` énumération.  
...
### <a name="bulk-insert-task"></a>tâche d'insertion en bloc  
 `DataFileType`Définie par la propriété à l’aide des valeurs de l' `DTSBulkInsert_DataFileType` énumération.  
...
### <a name="execute-sql-task"></a>Tâche d’exécution de requêtes SQL  
 `ResultSetType`Définie par la propriété à l’aide des valeurs de l' `ResultSetType` énumération.  
...
|ResultSetType_XML|4|  
 ...
 `SqlStatementSourceType`Définie par la propriété à l’aide des valeurs de l' `SqlStatementSourceType` énumération.  
...
### <a name="file-system-task"></a>Tâches du système de fichiers  
 `Operation`Définie par la propriété à l’aide des valeurs de l' `DTSFileSystemOperation` énumération.  
...
|DeleteDirectoryContent|9|  
 ...
 `Attributes`Définie par la propriété à l’aide des valeurs de l' `DTSFileSystemAttributes` énumération.  
...
### <a name="ftp-task"></a>Tâche FTP  
 `Operation`Définie par la propriété à l’aide des valeurs de l' `DTSFTPOp` énumération.  
...
### <a name="message-queue-task"></a>Message Queue Task  
 `MessageType`Définie par la propriété à l’aide des valeurs de l' `MQMessageType` énumération.  
...
|DTSMQMessagType_StringMessageToVariable|3|  
 ...
 `StringCompareType`Définie par la propriété à l’aide des valeurs de l' `MQStringMessageCompare` énumération.  
...
|DTSMQStringMessageCompare_Contains|3|  
 ...
 `TaskType`Définie par la propriété à l’aide des valeurs de l' `MQType` énumération.  
...
### <a name="send-mail-task"></a>tache Envoyer un message  
 `MessageSourceType`Définie par la propriété à l’aide des valeurs de l' `SendMailMessageSourceType` énumération.  
...
|Variable|2|  
 ...
 `Priority`Définie par la propriété à l’aide des valeurs de l' `MailPriority` énumération.  
...
### <a name="transfer-database-task"></a>Tâche de transfert de bases de données  
 `Action`Définie par la propriété à l’aide des valeurs de l' `TransferAction` énumération.  
...
|Déplacer|1|  
 ...
 `Method`Définie par la propriété à l’aide des valeurs de l' `TransferMethod` énumération.  
...
### <a name="transfer-error-messages-task"></a>Tâche de transfert de messages d'erreur  
 `IfObjectExists`Définie par la propriété à l’aide des valeurs de l' `IfObjectExists` énumération.  
...
|Ignorer|2|  
 ...
 `LoginsToTransfer`Définie par la propriété à l’aide des valeurs de l' `LoginsToTransfer` énumération.  
...
### <a name="transfer-sql-server-objects-task"></a>Tâche de transfert d'objets SQL Server  
 `ExistingData`Définie par la propriété à l’aide des valeurs de l' `ExistingData` énumération.  
...
### <a name="web-service-task"></a>Tâche de service Web  
 `OutputType`Définie par la propriété à l’aide des valeurs de l' `DTSOutputType` énumération.  
...
### <a name="wmi-data-reader-task"></a>Tâche Lecteur de données WMI  
 `OverwriteDestination`Définie par la propriété à l’aide des valeurs de l' `OverwriteDestination` énumération.  
...
|KeepOriginal|2|  
 ...
 `OutputType`Définie par la propriété à l’aide des valeurs de l' `OutputType` énumération.  
...
|PropertyNameAndValue|2|  
 ...
 `DestinationType`Définie par la propriété à l’aide des valeurs de l' `DestinationType` énumération.  
...
|Variable|1|  
 ...
 `WqlQuerySourceType`Définie par la propriété à l’aide des valeurs de l' `QuerySourceType` énumération.  
...
|Variable|2|  
 ...
 Propriété observateur d’événement WMI `ActionAtEvent` -défini à l’aide des valeurs de l' `ActionAtEvent` énumération.  
...
|LogTheEvent|1|  
 ...
 `ActionAtTimeout`Définie par la propriété à l’aide des valeurs de l' `ActionAtTimeout` énumération.  
...
|LogTimeout|1|  
 ...
 `AfterEvent`Définie par la propriété à l’aide des valeurs de l' `AfterEvent` énumération.  
...
|WatchfortheEventAgain|2|  
 ...
 `AfterTimeout`Définie par la propriété à l’aide des valeurs de l' `AfterTimeout` énumération.  
...
### <a name="xml-task"></a>Tâche XML  
 `OperationType`Définie par la propriété à l’aide des valeurs de l' `DTSXMLOperation` énumération.  
...
|Correctif|5|  
 ...
 `SourceType``SecondOperandType`Propriétés, et `XPathSourceType` définies à l’aide des valeurs de l' `DTSXMLSourceType` énumération.  
...
|DirectInput|2|  
 ...
 `DestinationType`et les propriétés **DiffGramDestinationType** -définies à l’aide des valeurs de l' `DTSXMLSaveResultTo` énumération.  
...
|Variable|1|  
 ...
 `ValidationType`Définie par la propriété à l’aide des valeurs de l' `DTSXMLValidationType` énumération.  
...
|XSD|1|  
 ...
 `XPathOperation`Définie par la propriété à l’aide des valeurs de l' `DTSXMLXPathOperation` énumération.  
...
|NodeList|2|  
 ...
 `DiffOptions`Définie par la propriété à l’aide des valeurs de l' `DTSXMLDiffOptions` énumération. Les options dans cet énumérateur ne sont pas mutuellement exclusives. Pour utiliser plusieurs options, fournissez une liste des options à appliquer, séparées par des virgules.  
...
|IgnoreDtd|128|  
 ...
 `DiffAlgorithm`Définie par la propriété à l’aide des valeurs de l' `DTSXMLDiffAlgorithm` énumération.  
...
### <a name="all-maintenance-tasks"></a>Toutes les tâches de maintenance  
 ...
 `DatabaseSelectionType`Définie par la propriété à l’aide des valeurs de l' `DatabaseSelection` énumération.  
...
|Spécifique|4|  
 ...
 `TableSelectionType`Définie par la propriété à l’aide des valeurs de l' `TableSelection` énumération.  
...
|Spécifique|2|  
 ...
 `ObjectTypeSelection`Définie par la propriété à l’aide des valeurs de l' `ObjectType` énumération.  
...
### <a name="back-up-database-task"></a>Tâche Sauvegarder la base de données  
 `DestinationCreationType`Définie par la propriété à l’aide des valeurs de l' `DestinationType` énumération.  
...
|Manuel|1|  
 ...
 `ExistingBackupsAction`Définie par la propriété à l’aide des valeurs de l' `ActionForExistingBackups` énumération.  
...
|Remplacer|1|  
 ...
 `BackupAction`Définie par la propriété à l’aide des valeurs de l' `BackupTaskType` énumération. Cette propriété définit avec la propriété `BackupIsIncremental` le type de sauvegarde que la tâche effectue.  
...
|Journal|2|  
 ...
 `BackupDevice`Défini par la propriété à l’aide de valeurs provenant de l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] énumération Smo (Management Objects) `DeviceType` .  
...
### <a name="maintenance-cleanup-task"></a>Tâche de nettoyage de maintenance  
 `FileTypeSelected`Définie par la propriété à l’aide des valeurs de l' `FileType` énumération.  
...
|FileReport|1|  
 ...
 `OlderThanTimeUnitType`Définie par la propriété à l’aide des valeurs de l' `TimeUnitType` énumération.  
...
### <a name="update-statistics-task"></a>Tâche Mettre à jour les statistiques  
 `UpdateType`Défini par la propriété à l’aide de valeurs provenant de l' [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] énumération Smo (Management Objects) `StatisticsTarget` .  
...
##  <a name="common-properties"></a><a name="CommonProperties"></a> Propriétés communes  
 ...
 `ForceExecutionResult`Définie par la propriété à l’aide des valeurs de l' `DTSForcedExecResult` énumération.  
...
|Completion|2|  
 ...
 `IsolationLevel`propriété définie par l' `IsolationLevel` énumération .NET Framework. Pour plus d’informations, consultez la bibliothèque de classes .NET Framework dans [MSDN Library](https://go.microsoft.com/fwlink?LinkId=17313).  
  
 `LoggingMode`Définie par la propriété à l’aide des valeurs de l' `DTSLoggingMode` énumération.  
...
|Désactivé|2|  
 ...
 `TransactionOption`Définie par la propriété à l’aide des valeurs de l' `DTSTransactionOption` énumération.  
