...
##  <a name="1-create-a-matrix-report-and-dataset-from-the-table-or-matrix-wizard"></a><a name="Setup"></a>1. créer un rapport de matrice et un DataSet à partir de l’Assistant tableau ou matrice  
...
3.  Ajoutez la [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` clause suivante comme dernière ligne de la requête :  
  
    ```  
    WHERE StoreID = (@StoreID)  
    ```  
  
     La `WHERE` clause limite les données récupérées à l’identificateur de magasin spécifié par le paramètre de requête *@StoreID* .  
...
8.  Dans le volet des données de rapport, développez le dossier **Paramètres** .  
 ...
 Notez qu’il existe désormais un paramètre de rapport nommé *@StoreID* . Par défaut, le type de données du paramètre est **texte**. Dans la mesure où l'identificateur de magasin est un entier, vous devez remplacer le type de données par le type Entier dans la procédure suivante.  
...
6.  Affichez un aperçu du rapport. La visionneuse de rapports affiche l’invite pour *@StoreID* .  
...
##  <a name="4b-specify-available-values-to-create-a-drop-down-list-of-values"></a><a name="AvailableValues"></a>4b. Spécifier les valeurs disponibles pour la création d'une liste déroulante de valeurs  
...
1.  Dans le volet données du rapport, cliquez avec le bouton droit sur le paramètre *@StoreID* , puis cliquez sur **Propriétés du paramètre**.  
...
9. Affichez un aperçu du rapport.  
  
     Dans la barre d’outils de la visionneuse de rapports, la zone de texte du paramètre est maintenant une liste déroulante qui s’affiche **\<Select a Value>** .  
...
##  <a name="4c-specify-default-values-so-the-report-runs-automatically"></a><a name="DefaultValues"></a>Quat. Spécifier des valeurs par défaut de sorte que le rapport s'exécute automatiquement  
...
7.  Affichez un aperçu du rapport.  
...
6.  Affichez un aperçu du rapport.  
...
##  <a name="4d-look-up-a-value-from-a-dataset-that-has-namevalue-pairs"></a><a name="NameValue"></a>4D. Rechercher une valeur à partir d'un dataset qui contient des paires nom/valeur  
...
8.  Affichez un aperçu du rapport.  
...
3.  À partir du volet données du rapport, faites glisser le paramètre *@StoreID* vers la zone de texte. La zone de texte affiche `[@StoreID]`.  
...
6.  Dans la zone **Valeur**, tapez `[@StoreID]`. `[@StoreID]` est la syntaxe d’expression simple qui représente `=Parameters!StoreID.Value`.  
...
16. Affichez un aperçu du rapport.  
...
8.  Dans la barre d’outils de la visionneuse de rapports, en regard de **afficher les sélections**, cliquez sur `True` .  
