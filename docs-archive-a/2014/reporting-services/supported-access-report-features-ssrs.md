...
## <a name="reports-with-custom-modules"></a>Rapports avec modules personnalisés  
 Si [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] du code personnalisé est contenu dans des modules, il n’est pas converti. Si Concepteur de rapports rencontre du code pendant le processus d’importation, un avertissement est généré et affiché dans la fenêtre **liste des tâches** .  
...
|maintenant|Seconde|Temps|Temps$|  
