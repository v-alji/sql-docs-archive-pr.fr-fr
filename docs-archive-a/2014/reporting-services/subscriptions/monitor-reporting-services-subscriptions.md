...
|Terminé : \<*number*> traitement du \<*number*> total ; \<*number*> Erreurs.|Indique l'état de l'exécution d'un abonnement piloté par les données. Ce message provient du processeur de planification et de livraison.|  
...
|\<*number*>nombre|Nombre total de notifications générées pour la dernière remise de l'abonnement.|  
...
|Le fichier \<*filename*> a été écrit \<path> .|Indique que la remise à l'emplacement du partage de fichiers a réussi ; ce message provient de l'extension de remise dans un partage de fichiers.|  
...
|Échec de la connexion au dossier de destination, \<path> . Assurez-vous que le dossier de destination ou le partage de fichiers existe.|Indique que le dossier que vous avez spécifié n'a pas pu être trouvé ; ce message provient de l'extension de remise dans un partage de fichiers.|  
|Impossible \<filename> d’écrire dans le fichier \<path> . Nouvelle tentative d'écriture en cours.|Indique que le fichier n'a pas pu être mis à jour avec une version plus récente ; ce message provient de l'extension de remise dans un partage de fichiers.|  
|Échec lors de l’écriture du fichier \<filename> :\<message>|Indique que la remise à l'emplacement du partage de fichiers a échoué ; ce message provient de l'extension de remise dans un partage de fichiers.|  
...
### <a name="native-mode-log-files"></a>Fichiers journaux en mode natif  
 ...
 Les administrateurs du serveur de rapports peuvent consulter les fichiers **ReportServerService_ \* . log** pour déterminer l’état de remise de l’abonnement. Pour la remise par messagerie électronique, les fichiers journaux du serveur de rapports contiennent un enregistrement du traitement et des remises effectuées sur les comptes de messagerie spécifiques. Voici l'emplacement par défaut des fichiers journaux :  
