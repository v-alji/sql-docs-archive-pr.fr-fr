...
-   Vérifiez que vous avez ouvert une session en tant que compte d'utilisateur qui dispose d'autorisations `sysadmin` sur l'instance de [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].
...
5.  Entrez le nom de serveur du moteur de base de données. Si vous avez attaché les bases de données du serveur de rapports à une instance nommée, vous devez taper le nom de l’instance au format suivant : \<servername> \\<InstanceName \> .
