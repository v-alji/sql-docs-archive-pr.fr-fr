...
-   Code personnalisé dans l' **\<Code>** élément d’une définition de rapport.  
...
|`AllowNew`|Attribut booléen pour **allow** qui contrôle si la création de nouvelles instances du type est autorisée dans les expressions RDL ou dans un **\<Class>** élément RDL.<br /><br /> Remarque : lorsque `RDLSandboxing` est activé, les nouveaux tableaux ne peuvent pas être créés dans les expressions RDL, quel que soit le paramètre de `AllowNew` .|  
...
|**Membres**|Pour obtenir la liste des types qui sont inclus dans l' **\<Types>** élément, la liste des noms de membres qui ne sont pas autorisés dans les expressions RDL.|  
...
-   Ajoutez un **\<Deny>** élément pour le nom de membre.  
...
#### <a name="maintaining-the-deny-list-of-members"></a>Gestion \<Deny> de la liste des membres  
...
### <a name="working-with-operators-and-new"></a>Utilisation des opérateurs et de New  
 Par défaut, les opérateurs de langage [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] .NET Framework, à l'exception de `New`, sont toujours autorisés. L' `New` opérateur est contrôlé par l' `AllowNew` attribut sur l' **\<Allow>** élément. D’autres opérateurs de langage, tels que l’opérateur d’accesseur de collection par défaut `!` et les [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] macros de cast .NET Framework telles que `CInt` , sont toujours autorisés.  
