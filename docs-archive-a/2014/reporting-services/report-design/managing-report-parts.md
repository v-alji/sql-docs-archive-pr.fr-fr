...
### <a name="server-side-interaction-and-search"></a>Interaction et recherche côté serveur  
 Les parties de rapports peuvent être publiées sur un serveur de rapports en mode natif ou en mode intégré SharePoint. Les utilisateurs peuvent utiliser la fonctionnalité de la bibliothèque de parties de rapports dans une application de création de rapports telle que [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Générateur de rapports pour rechercher et ajouter des parties de rapports à leurs rapports. Lorsqu'un utilisateur recherche une partie de rapport, la recherche regarde le catalogue du serveur de rapports indépendamment du mode pour lequel le serveur a été installé.  
...
|Ajouter, supprimer et télécharger des parties de rapport|Publisher|  
