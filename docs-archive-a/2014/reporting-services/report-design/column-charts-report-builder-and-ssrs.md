...
-   La possibilité d'afficher votre graphique sous la forme d'un histogramme ou diagramme de Pareto est une spécificité des histogrammes. Pour ce faire, affectez à la propriété ShowColumnAs a la valeur `Histogram` ou `Pareto` dans la fenêtre Propriétés `true` .  
