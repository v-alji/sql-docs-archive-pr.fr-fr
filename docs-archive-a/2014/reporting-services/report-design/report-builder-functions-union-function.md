...
## <a name="return"></a>Renvoie  
 Retourne un objet spatial, `SqlGeometry` ou `SqlGeography`, selon le type d'expression. Pour plus d’informations sur `SqlGeometry` les `SqlGeography` types de données spatiales et, consultez [vue d’ensemble des types de données spatiales](../../relational-databases/spatial/spatial-data-types-overview.md).  
