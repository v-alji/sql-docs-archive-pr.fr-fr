...
1.  Ouvrez le fichier **RSReportServer.config** dans un éditeur de texte. Par défaut, il se trouve dans le dossier \Program Files\Microsoft SQL Server\MSRS12. \<*instancename*> \Reporting Services\ReportServer.  
...
4.  Sur l'ordinateur Gestionnaire de rapports, modifiez les paramètres de configuration dans le fichier RSReportServer.config de manière à pointer sur l'instance distante du serveur de rapports. Au démarrage, le Gestionnaire de rapports lira le fichier de configuration afin d'obtenir l'URL du serveur de rapports :  
    ...
    1.  Ouvrez RSReportServer.config dans un éditeur de texte. Par défaut, il se trouve dans le dossier \Program Files\Microsoft SQL Server\MSRS11. \<*instancename*> \Reporting Services\ReportServer.  
...
3.  Entrez l'URL du Gestionnaire de rapports. Par défaut, il s’agit de http:// \<**your-server-name**> /reports, mais si vous avez installé Reporting Services en tant qu’instance nommée, l’URL par défaut sera http:// \<**your-server-name**> /reports \<**_instancename**> .  
...
1.  Ouvrez le fichier RSReportServer.config dans un éditeur de texte. Par défaut, il se trouve dans le dossier \Program Files\Microsoft SQL Server\MSRS11. \<*instancename*> \Reporting Services\ReportServer.  
