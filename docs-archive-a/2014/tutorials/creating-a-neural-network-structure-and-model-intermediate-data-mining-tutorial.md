...
11. Sélectionnez la **clé**, l' **entrée**et les `Predict` cases à cocher comme indiqué dans le tableau suivant :  
    ...
    |Orders (Commandes)|Entrée/Prédire|  
...
12. Dans la page **spécifier le type de contenu et de données des colonnes** , vérifiez que la grille contient les colonnes, les types de contenu et les types de données, comme indiqué dans le tableau suivant, puis cliquez sur **suivant**.  
    ...
    |Orders (Commandes)|Continue|Long|  
...
14. Dans la page **fin de l’Assistant** , pour **nom**de la structure d’exploration de données, tapez `Call Center` .  
  
15. Pour le **nom du modèle d’exploration de données**, tapez `Call Center Default NN` , puis cliquez sur **Terminer**.  
...
4.  Dans la boîte de dialogue **nouveau modèle d’exploration de données** , pour **nom du modèle**, tapez `Call Center Binned NN` . Dans la liste déroulante nom de l' **algorithme** , sélectionnez **Microsoft neuronal Network**.  
...
5.  Cliquez n’importe où dans l’onglet **modèle d’exploration de données** .  
     ...
     La grille est mise à jour pour afficher le nouvel alias de colonne temporaire, `ServiceGrade` , en regard de l’utilisation de la colonne. La grille contenant la structure d'exploration de données et deux modèles d'exploration de données doit ressembler à ce qui suit :  
    ...
    |Orders (Commandes)|Entrée|Entrée|  
