...
6.  Dans le volet **modèle d’exploration de données** , sélectionnez la table imbriquée et faites-la `v Assoc Seq Line Items` glisser dans la grille vers la zone **critères/argument** de la fonction **PredictSequence** .  
...
2.  Dans la boîte de dialogue **entrée de requête singleton** , cliquez sur la zone **valeur** pour `Region` , puis sélectionnez la ligne vide pour effacer l’entrée pour ce champ.  
...
5.  Dans la nouvelle ligne, cliquez sur la zone sous `Model` , puis sélectionnez Touring Band dans la liste. Cliquez sur **OK**.  
