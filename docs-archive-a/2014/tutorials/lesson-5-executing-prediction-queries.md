...
# <a name="lesson-5-executing-prediction-queries"></a>Leçon 5 : Exécution des requêtes de prédiction
  Dans cette leçon, vous allez utiliser la forme [Select from \<model> PREDICTION JOIN (DMX)](/sql/dmx/select-from-model-cases-dmx) de l’instruction SELECT pour créer deux types différents de prédictions en fonction du modèle d’arbre de décision que vous avez créé au cours de la [leçon 2 : ajout de modèles d’exploration de données à la structure d’exploration de données d’association](../../2014/tutorials/lesson-2-adding-mining-models-to-the-market-basket-mining-structure.md). Ces types de prédiction sont définis ci-dessous.  
...
```  
 ...
 Tout comme dans la requête singleton, les deux premières lignes du code définissent les colonnes du modèle d'exploration de données que retourne la requête, ainsi que le nom du modèle d'exploration utilisé pour générer la prédiction. L' \<number> instruction Top spécifie que la requête retourne uniquement le nombre ou les résultats spécifiés par \<number> .  
