...
# <a name="lesson-4-executing-market-basket-predictions"></a>Leçon 4 : Exécution de prédictions Market Basket
 ...
 Le formulaire **Sélectionner à partir d’une \<model> jointure de prédiction** de l' `SELECT` instruction contient trois parties :  
...
## <a name="singleton-prediction-join-statement"></a>Instruction singleton PREDICTION JOIN  
 La première étape consiste à créer une requête singleton à l’aide de la syntaxe **Select from \<model> PREDICTION JOIN** et en fournissant un ensemble unique de valeurs comme entrée. L'exemple générique suivant utilise l'instruction singleton :  
...
1.  Dans l' **Explorateur d’objets**, cliquez avec le bouton droit sur l’instance de [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] , pointez sur **nouvelle requête**, puis cliquez sur **DMX** pour ouvrir l’éditeur de requête.  
