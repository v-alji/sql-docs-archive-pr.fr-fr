...
2.  Dans la `PERIODICITY_HINT` ligne de la boîte de dialogue **paramètres d’algorithme** , cliquez sur la colonne **valeur** , puis tapez `{12}` , y compris les accolades.  
  
     Par défaut, l'algorithme ajoute également la valeur {1}.  
  
3.  Dans la `FORECAST_METHOD` ligne, vérifiez que la zone de texte **valeur** est vide ou a la valeur `MIXED` . Si une valeur différente a été entrée, tapez `MIXED` pour rétablir la valeur par défaut du paramètre.  
  
4.  Dans la ligne **PREDICTION_SMOOTHING** , vérifiez que la zone de texte **valeur** est vide ou définie sur 0,5. Si une valeur différente a été entrée, cliquez sur **valeur** et tapez pour rétablir la `0.5` valeur par défaut du paramètre.  
...
## <a name="handling-missing-data-optional"></a>Gestion des données manquantes (facultatif)  
 ...
 «Erreur (exploration de données) : horodatages non synchronisés à partir \<series name> de la série, du modèle d’exploration de données, \<model name> . Toutes les séries chronologiques doivent se terminer à la même marque de temps et ne peuvent pas comporter des points de données arbitrairement absents. L'attribution de la valeur Previous ou de la valeur d'une constante numérique au paramètre MISSING_VALUE_SUBSTITUTION fournit automatiquement les points de données absents lorsque cela est possible. »  
...
2.  Dans la boîte de dialogue **paramètres d’algorithme** , dans la ligne **MISSING_VALUE_SUBSTITUTION** , cliquez sur la colonne **valeur** , puis tapez `Mean` .  
